...
export default function ElibLogin(){
    ...
    const [Email, setEmail] = useState('ismael@itca.edu.sv');
            ...
            <View className="h-full w-full flex justify-around pt-40 pb-10">
                ...
                <View className="flex items-center mx-4 space-y-4">
                    <Animated.View entering={FadeInDown.duration(1000).springify()} className="bg-white/60 p-5 rounded-2xl w-full shadow-xl">
                        <TextInput placeholder='Email' placeholderTextColor={'gray'} value={Email} onChangeText={(text) => {
                            setEmail(text);
                            setErrors([])
                        }}/>
                    </Animated.View>
                    ...
                    <Animated.View  entering={FadeInDown.delay(400).duration(1000).springify()} className='w-full'>
                        <TouchableOpacity className={`w-full bg-green-600 p-3 rounded-2xl mb-3 ${loading && 'bg-gray-400 disabled:cursor-not-allowed'}`} onPress={handleLogin}>
